...
class ListMoviePagedAdapter(public val context: Context): PagedListAdapter<Movie,RecyclerView.ViewHolder>(MovieDiffCallback()) {
    ...
    fun setNetworkState(newNetworkState: NetworkState){
        ...
        if(hadExtraRow != hasExtraRow){
            if(hadExtraRow){
                notifyItemRemoved(super.getItemCount()) //se quita el progressbar del final cuando no hay mas items que cargar
            }else{
                notifyItemInserted(super.getItemCount())
            }
        }else if(hasExtraRow && previousState != newNetworkState){  //ERROR
